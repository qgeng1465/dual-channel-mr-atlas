...
For every gene–trait pair with a valid MR estimate, we applied coloc (coloc.abf) [@giam2014] with prior probabilities p1 = p2 = 1e-4 and p12 = 1e-5, using region-level summary statistics in the LD block containing the lead instrument. Colocalization support was reported as PP.H4 (probability of a single shared causal variant). Strong colocalization was defined as PP.H4 ≥ 0.8. For a sensitivity set of six representative candidates spanning different signal architectures, we additionally ran coloc.susie with SuSiE fine-mapping [@wang2020][@wallace2021] under a multiple-causal-variant assumption. The analysis used external 1000 Genomes EUR LD with equal-prior-variance (EPV = FALSE) and explicit sample size. Because SuSiE is sensitive to imperfectly matched reference panels, convergence and PP.H4 were additionally assessed under a ridge-shrunk LD matrix R_w = (1−w)R + wI (Methods 2.7), and the six adjudicated loci were cross-checked with the hyprcoloc algorithm [@foley2021] and conditional/joint fine-mapping with GCTA-COJO [@yang2012] (Methods 2.7; Results 3.7). The one adjudicated locus (LAMC1×CAD) was decided on multi-signal fine-mapping evidence independent of the coloc posterior (Section 3.7).
...
### 2.7 Robustness layer for strong-colocalization calls

For the six adjudicated loci, three complementary fine-mapping and colocalization methods were applied in addition to coloc.abf. (i) coloc.susie with SuSiE fine-mapping [@wang2020][@wallace2021] was run under external 1000 Genomes EUR LD with equal-prior-variance (EPV = FALSE) and explicit sample size (n = 31,684 for eQTLGen; n = 655,666/296,525 for T2D/CAD). Because SuSiE is sensitive to imperfectly matched reference panels, we swept the ridge-shrunk LD matrix R_w = (1−w)R + wI for w ∈ {0.05, 0.10, 0.20} and recorded per-side convergence and PP.H4 at each weight. (ii) hyprcoloc (v1.0) [@foley2021], which allows one or more shared causal variants across traits, was run on the same regional SNP sets (eQTLGen blood expression and the outcome GWAS, two traits) using the 1000 Genomes EUR LD matrix and priors π1 = 1e-4, πc = 0.02 and π12 = 1e-5, reporting the shared-variant posterior (PP.shared) and regional probability. (iii) Conditional and joint (COJO) analysis with GCTA (v1.94.1) [@yang2012] performed stepwise selection within each locus at two thresholds (p < 5e-8, genome-wide; p < 1e-4, locus-level) using 1000 Genomes EUR genotypes, extracting the number of independent signals and the conditional result of the candidate lead instrument given the region's selected signals. Regional SNP sets were intersected with the reference bfile by rsID to avoid GRCh38 (GWAS summary)/hg19 (reference) coordinate mismatch.

### 2.8 Directionality: MR-Steiger with measurement-error correction and empirical-Bayes coloc enrichment

For each candidate lead instrument, directionality was tested by the MR-Steiger method [@hemani2017] with per-SNP r² = 2p(1−p)β² / (2p(1−p)β² + n·se²), where p is the effect-allele frequency and β, se and n the effect, standard error and sample size of the exposure (eQTLGen; β = Z/√n, se = 1/√n) and outcome (T2D/CAD GWAS) associations. Because eQTLGen (n ≈ 26k) and the outcome GWAS (n ≈ 297k–656k) differ in sample size by an order of magnitude, the direction test used independent-correlation Fisher z-statistics z = (atanh(r_exp) − atanh(r_out)) / √(1/(n_exp−3) + 1/(n_out−3)), with the corrected direction defined as r²_exp > r²_out. In the style of TORUS [@wen2016] and fastENLOC [@weissbrod2020][@hormozdiari2016], we additionally estimated an empirical-Bayes enrichment prior for strong colocalization: the background rate of strong coloc across all tested pairs (π0) versus the rate among MR-significant pairs (π_mr) gives an enrichment ratio ρ = π_mr/π0 and an enrichment-adjusted posterior PP.H4_enr = ρ·PP.H4 / (1 + (ρ−1)·PP.H4). The 121 strong-colocalization pairs were re-assessed at PP.H4_enr ≥ 0.8.

### 2.9 Gene-regulatory network proximity

Network propagation by random walk with restart (r = 0.7, convergence 1e-8) was run on the STRING v12 human protein–protein interaction network (9606.protein.links.v12.0; confidence thresholds ≥ 400 and ≥ 700 as a sensitivity sweep) [@szklarczyk2023], seeded from the 106 previously reported effector genes. Each of the 15 candidate effector genes was scored by its RWR score, its rank among network nodes, and its Louvain module membership relative to the seed module. Statistical significance of the candidate set's proximity to the seed module was evaluated by permutation (100 random equal-size seed sets; statistic = maximum candidate RWR score), reported as a permutation p-value and z-score.

### 2.10 Cross-tissue and cell-type mapping

For each candidate effector gene, GTEx v8 per-gene lead-eQTL files [@gtex2020] were queried across eight tissues (whole blood, subcutaneous and visceral adipose, liver, pancreas, skeletal muscle, coronary artery, aorta). For each tissue, we recorded whether a significant lead eQTL existed and whether the tissue lead variant matched the eQTLGen blood lead (identical variant or within ±50 kb). Cell-type enrichment of the 121 strong-colocalization genes and the 15 candidate effector genes was tested against PanglaoDB marker genes [@franzen2019] (4,345 human genes, 178 cell types) by hypergeometric enrichment relative to the human gene background, with Benjamini–Hochberg FDR correction across cell types. This is a marker-gene enrichment analysis, not a single-cell projection: it maps where the effector genes are most expressed at cell-type level but cannot resolve cell-type-specific eQTL architecture.

### 2.11 PheWAS pleiotropy screen and pQTL coverage

A systematic phenotype-wide screen was run for the 15 candidate lead instruments against the full GWAS Catalog (curated at P < 5e-8) [@sollis2023], classifying each hit by domain (cardiac, diabetes, lipid, blood-pressure, platelet, immune, other) to quantify cross-domain pleiotropy of each candidate instrument. Protein-level (pQTL) coverage of the 15 candidates was assessed against the deCODE [@ferkingstad2021] and UKB-PPP [@sun2023] protein panels (OpenGWAS prot-a and ukbpp tracks); coverage was scored as present/absent per candidate.
...
To test robustness of the strong-colocalization calls to the single-causal-variant assumption of coloc.abf, we applied three complementary fine-mapping and colocalization methods to the six adjudicated loci spanning different signal architectures (RBM6×T2D, CNNM2×CAD, PLAUR×CAD, CD101×T2D, RIC8A×CAD, LAMC1×CAD; Methods 2.7).

Under unshrunken external 1000 Genomes EUR LD, coloc.susie with SuSiE fine-mapping fully converged on both sides for only 1/6 loci (RIC8A×CAD); RBM6 errored and the remaining four had at least one non-converged side. Because non-convergence under external LD is expected when summary statistics and reference panels are imperfectly matched, we applied the ridge-shrunk LD remedy R_w = (1−w)R + wI, where I is the identity (Methods 2.7). At w ≥ 0.05 all six loci converged on both sides, and PP.H4 reached ≈1.0 for five loci (RBM6 1.0000, CD101 1.0000, CNNM2 0.9994, RIC8A 1.0000, LAMC1 1.0000), consistent with the coloc.abf values (0.9448, 0.9488, 0.9366, 0.8895 and 0.9139, respectively). PLAUR×CAD was the single exception: PP.H4 collapsed from 0.9990 (w = 0) to 0.4134 (w = 0.05) and 0.2031 (w = 0.10), and was undefined at w = 0.20 (no GWAS credible sets remaining), revealing a coloc signal that is fragile to the LD prior.

An independent two-trait hyprcoloc analysis (Methods 2.7) largely corroborated coloc.abf: PP.shared was 0.8771 (CD101), 0.8571 (CNNM2), 0.7765 (RBM6), 0.9511 (PLAUR) and 0.5757 (LAMC1); RIC8A dropped the eQTLGen trait (regional probability 0.45), consistent with its multi-signal architecture. Notably, hyprcoloc supports the coloc.abf call for PLAUR (PP.shared = 0.9511 versus PP.H4 = 0.9957) rather than the ridge-shrunk SuSiE posterior (0.4134), indicating that PLAUR's fragility is specific to the SuSiE algorithm under external LD rather than to the shared-causality evidence per se. Conditional and joint (COJO) fine-mapping with GCTA (Methods 2.7) identified 13 independent signals at locus-level significance (p < 1e-4) across the six regions (RBM6 4, CD101 4, CNNM2 2, PLAUR 1, RIC8A 1, LAMC1 1), of which 5 reached genome-wide significance (p < 5e-8; CD101 4, CNNM2 1; Fig S1). Only the PLAUR lead instrument (rs4760) itself indexed an independent signal (joint p = 2.0e-6); the remaining leads were either col-linear with a selected signal (RBM6, CNNM2; conditional p undefined), excluded by the collinearity filter (CD101), or non-significant conditional on the region index signal (RIC8A p = 0.71, LAMC1 p = 0.66). COJO therefore corroborates the coloc.susie and hyprcoloc classification for the multi-signal loci and independently confirms the PLAUR call.

The one locus with clear multi-signal architecture, LAMC1×CAD, was independently excluded by the FDR filter (padj ≥ 0.05): fine-mapping revealed eight independent eQTL credible sets versus three weak GWAS credible sets (maximum z = 4.32) with no shared variant, and its hyprcoloc posterior was the weakest of the six (0.5757). In multi-signal regions with weak GWAS evidence, coloc.abf can overstate PP.H4. The retained 15 candidates therefore do not depend on the single-causal-variant assumption for their support: they were selected on the primary coloc.abf analysis and, where testable, survived ridge-shrunk SuSiE, hyprcoloc and directional replication in GTEx and FinnGen. PLAUR×CAD is retained as a candidate, but its coloc support is the most assumption-sensitive of the set (Fig S1); its downstream support (MR-Steiger direction, GTEx and FinnGen replication, Section 3.6) does not depend on the coloc prior.
...
### 3.9 Method robustness and orthogonal validation of the candidate set

Five additional analyses evaluated the robustness and orthogonal validity of the strong-colocalization candidate set (Methods 2.7–2.11; Fig S1–S6).

**Ridge-shrunk SuSiE and hyprcoloc.** The robustness layer (Section 3.7; Fig S1) recovered SuSiE convergence under ridge-shrunk LD for all six adjudicated loci and corroborated coloc.abf for five of them, with PLAUR×CAD the single assumption-sensitive exception.

**Directionality with measurement-error correction.** After correcting for the ~10-fold sample-size asymmetry between eXposure (eQTLGen) and outcome GWAS (Methods 2.8), 15/15 candidate lead instruments showed eQTL → outcome direction (r²_exp > r²_out), with 14/15 significant at the corrected Fisher-z test (Fig S2). Two sign flips relative to the stored MR effect (RIC8A, PLAUR) were allele-convention artifacts with identical effect magnitudes (Methods 2.8).

**Empirical-Bayes coloc enrichment.** The background rate of strong colocalization across the 24,863 coloc-evaluable pairs (127/24,863) was π0 = 0.0051; among the 982 MR-significant pairs it was π_mr = 0.1232, an enrichment ratio ρ = 27.4 (95% CI 21.1–35.4). Applying this prior as an enrichment-adjusted posterior (Methods 2.8) retained all 121 strong-colocalization pairs at PP.H4_enr ≥ 0.8 (121/121), i.e., no MR-significant pair changed classification under the empirical-Bayes enrichment prior (Fig S2).

**Gene-regulatory network proximity.** RWR propagation from the 106 known effector genes on the STRING v12 network gave no enrichment of the 15 candidates for proximity to the known-gene module: the maximum candidate RWR score was not distinguishable from the permutation null (permutation p = 0.19 at confidence ≥ 400; p = 0.62 at ≥ 700), and 2/15 candidates (CCDC19, U6atac) were not represented in the network (Fig S3). This is an honest negative: the candidate genes are, by construction, absent from the known-locus annotation and are not network-close to it.

**Cross-tissue lead-eQTL concordance.** GTEx v8 lead-eQTL evidence was available for 14/15 candidates (U6atac, a non-coding RNA, is not tested as a lead eQTL). All 14 had a significant lead eQTL in ≥1 of 8 tissues, and 10/14 had the tissue lead variant within ±50 kb of the eQTLGen blood lead in ≥1 tissue; RIC8A was concordant in all 7 of its significant tissues, whereas RBM6, CNNM2, VSIG8 and SLC12A3 showed no concordant tissue lead (Fig S4).

**PheWAS pleiotropy screen.** Among the 13 unique candidate lead instruments, 5 had no GWAS Catalog entry at P < 5e-8, 2 were cardiometabolic-only (CNNM2, U6atac), and 6 crossed trait domains. PLAUR was the most pleiotropic (85 trait associations, 37 studies, spanning thrombosis and cardiovascular traits), and SLC12A3 associated with 25 traits, predominantly lipid and protein measures (Methods 2.11; Fig S5). Cross-domain signal does not invalidate a candidate but tempers effector-gene specificity claims (Discussion).

**pQTL coverage and cell-type enrichment.** None of the 15 candidates had a measured pQTL in the deCODE or UKB-PPP panels (0/15; Methods 2.11), a data-availability limitation. Cell-type enrichment against PanglaoDB markers found no cell type with FDR < 0.05 among either the 121 known or the 15 candidate genes; the strongest nominal signals (SLC12A3 → distal tubular cells, p = 0.14; CLEC3B → microglia, p = 0.22 and fibroblasts, p = 0.45) are consistent with known biology but do not survive multiple-testing control (Fig S6).
...
**Supplemental Figure S1. Method robustness of strong-colocalization calls.** (A) Ridge-shrunk LD sweep for the six adjudicated loci: coloc.susie PP.H4 as a function of the ridge weight w in R_w = (1−w)R + wI. All six loci converge at w ≥ 0.05 with PP.H4 ≈ 1.0 except PLAUR×CAD, whose PP.H4 collapses from 0.9990 (w = 0) to 0.4134 (w = 0.05) and 0.2031 (w = 0.10), and is undefined at w = 0.20 (no remaining GWAS credible set). (B) Hyprcoloc cross-check: PP.shared (probability of a shared causal variant) versus coloc.abf PP.H4 for the six loci. PLAUR PP.shared = 0.9511 corroborates coloc.abf; RIC8A dropped the eQTLGen trait (regional probability 0.45), consistent with its multi-signal structure. (C) GCTA-COJO conditional/joint fine-mapping: independent signals at p < 5e-8 (genome-wide) and p < 1e-4 (locus-level) per locus, with the candidate lead instrument's conditional status annotated. LAMC1×CAD (abf PP.H4 = 0.9139) is excluded from the candidate set on independent FDR and multi-signal grounds.

**Supplemental Figure S2. Directionality and empirical-Bayes coloc enrichment.** (A) MR-Steiger measurement-error-corrected r² for the 15 candidate lead instruments: r²_exposure (eQTLGen, n ≈ 26k) versus r²_outcome (T2D/CAD GWAS, n ≈ 297k–656k). All 15 show r²_exp > r²_out (eQTL → outcome); asterisks mark corrected Fisher-z p < 0.05 (14/15; CNNM2 is the sole non-significant case, p = 0.13). (B) Enrichment-adjusted posterior PP.H4_enr versus coloc.abf PP.H4 for the 121 strong-colocalization pairs under an empirical-Bayes enrichment prior (ρ = 27.4 [95% CI 21.1–35.4]; background π0 = 0.0051, MR-significant rate π_mr = 0.1232). All 121 pairs remain at PP.H4_enr ≥ 0.8.

**Supplemental Figure S3. Gene-regulatory network proximity of the 15 candidate effector genes.** (A) RWR scores (STRING v12, restart 0.7, confidence ≥ 400) for the 15 candidates against the permutation null; permutation p = 0.19, z = −0.26. (B) Network subgraph around the 106 known seeds (blue) and the 15 candidates (red stars); CCDC19 and U6atac are absent from the network.

**Supplemental Figure S4. Cross-tissue lead-eQTL concordance for the 15 candidate effector genes** (GTEx v8 per-gene lead eQTL, eight tissues). Fill = −log10(q) of the per-tissue lead eQTL; red ring = tissue lead within ±50 kb of the eQTLGen blood lead; grey dot = not concordant; blank = no lead-eQTL test in tissue. 14/15 candidates have ≥1 significant tissue lead eQTL and 10/14 are concordant in ≥1 tissue; U6atac is not tested as a GTEx lead eQTL.

**Supplemental Figure S5. PheWAS pleiotropy screen of the 15 candidate lead instruments** against the full GWAS Catalog (curated at P < 5e-8). Volcano of trait associations per lead SNP (−log10(P) versus effect), colored by trait domain. PLAUR is the most pleiotropic lead (85 traits, 37 studies), SLC12A3 associates with 25 traits (mostly lipid and protein measures); 5/13 unique lead SNPs have no catalog entry.

**Supplemental Figure S6. Cell-type enrichment of strong-colocalization genes** against PanglaoDB markers (4,345 human genes, 178 cell types). Hypergeometric enrichment −log10(p) per cell type for the 121 known (top) and 15 candidate (bottom) effector genes; no cell type passes BH-FDR < 0.05. Nominal maxima: SLC12A3 → distal tubular cells, CLEC3B → microglia/fibroblasts.
