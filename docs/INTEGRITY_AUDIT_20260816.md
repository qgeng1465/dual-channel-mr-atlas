# 对抗性学术诚信审计 + 历史评审与技术评估记录（2026-08-16）

> 本文档 = 审计与评审记录汇总（3 个文档合并；原件已归档 `docs/archive/`）：
> - **正文 §0–§七** = 对抗性学术诚信审计（原 `docs/INTEGRITY_AUDIT_20260816.md`）
> - **附录 A** = 历史对抗性评审记录（原 `docs/archive/verification_20260815.md`，驱动 08-15 破局方案修订）
> - **附录 B** = coloc.susie 收敛性评估（原 `docs/archive/COLOC_SUSIE_ASSESSMENT_20260816.md`）

---

# 对抗性学术诚信审计报告 — dual-channel MR atlas（2026-08-16）

> 审计对象：`<repo-root>`（目标投稿 HMG，方法学+图谱资源论文）
...
- **为什么是 P0**：这是论文最核心的定量声称。只报 12.9%（更"好"的数字）而不披露同管线直接算出的 3.0%（更"差"的数字），审稿人一跑数就会被抓成"口径不一致/选择分母"，且属于对不利读数的不披露。文档自己最新一份评审建议（`docs/PAPER_WRITING_PLAN_20260816.md（附录 C）` §0/§2.3/§6 第 8 条）也明确把它列为"必须主动披露，否则被当把柄"，并建议并排报两个 precision 做成漏斗图——说明项目已意识到，但尚未写进任何论文面向文档。
- **改法**：论文并排报告 `扫描层精度 129/4248=3.0% [2.56–3.60%]` 与 `stage-2 复核层精度 106/818=12.96% [10.83–15.43%]`（2026-08-16 口径统一：819 中 TCF19×CAD 因 GWAS 区域数据不可用无法评估 coloc，精度分母用可评估的 818 对），写清各自分母与漏斗关系（4248 ⊃ 819 ⊃ 818；129 = 已知 106 + 新 23），把"复核步骤把共定位支持率提升 ~4 倍"做成明确结论（这正是 `docs/PAPER_WRITING_PLAN_20260816.md` 附录 C 给的诚实卖点）。删除"唯一真读数"这类措辞。
...
- 131 strong 是 31,373 次 coloc 检验的调用；MR 用 BH-FDR（stage-2 812/819），但 coloc 的 PP.H4≥0.8 调用没有家族级显著性声明，置换标定也仅覆盖 MR 显著子集。论文需明确"图谱为描述性/假设生成性质，不做家族级显著性宣称"（`docs/PAPER_WRITING_PLAN_20260816.md` 附录 C §4 第 11 条已有预案，需落实到正文）。
...
| P2-3 | 附录 A §10b（line 99） | 报 M22 旧 55%/45% 未标注"已被 build 错配修复推翻"（修正只在 feasibility §7.8 / CHANGELOG） | 55%=58/106 为错配值 |
| P2-4 | 附录 A §6（line 55） | "76 基因口径错误：实际 56 编码 + 20 非编码"已被 M21 推翻（正确 76 编码 + 30 非编码）；附录 A 文档未加更正注 | ukbpp_coverage 76 行全匹配 |
...
- **6000 抽样 → 全量枚举的不一致是否如实**：是，如实。M20 抽样 0/6000 与全量 t2d 仅 AP3S2（+cad ZNF19）的差异被明确归因为"抽样未抽到 AP3S2"（feasibility §7.7 line 150），并改为"极罕见（全量 t2d 1/10,190=0.01%）"而非"严格 0"。Poisson 上界（`docs/PAPER_WRITING_PLAN_20260816.md` 附录 C：2/31,373 的 95% CI 上界 ≈0.023%/对）口径合理。
...
## 附录 A — 历史对抗性评审记录：破局方案验证（原 verification_20260815.md）
...
## 附录 B — coloc.susie 收敛性评估（原 COLOC_SUSIE_ASSESSMENT_20260816.md）
