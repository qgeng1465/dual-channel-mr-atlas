#!/bin/bash
# =============================================================================
# M34b_coloc_susie_maxiter1000.sh — coloc.susie 敏感性（max_iter=1000 重跑）
# =============================================================================
...
# 输出：results/m34b_coloc_susie_maxiter1000_20260816.csv
...
# 500 迭代仍 conv=FALSE）。审稿意见要求提高收敛性：取 max_iter=1000 重跑，
# 收敛标志如实写入 note；若仍不收敛则按方案 B（exploratory + 多信号证据）处理。
f1 <- tryCatch(susie_rss(z1, R, n = 31684, L = 10, estimate_residual_variance = FALSE, coverage = 0.9, max_iter = 1000),
               error = function(e) NULL)
f2 <- tryCatch(susie_rss(z2, R, n = if (out == "t2d") 655666 else 296525, L = 10,
                         estimate_residual_variance = FALSE, coverage = 0.9, max_iter = 1000),
               error = function(e) NULL)
