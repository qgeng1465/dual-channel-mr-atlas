#!/usr/bin/env python3
# =============================================================================
# M33_figures_upgrade_ajhg_20260816.py — 主图 AJHG 升级：Fig 1/2/3/4/6/7
# =============================================================================
# 按 figure 调研结论升级：
#   Fig 1  浓缩工作流示意图（编辑第一眼）——31,373→131→23→双复现
#   Fig 2  精度漏斗 + 95% Wilson CI（操作特性曲线，Kutalik/Zuber 背书）
#   Fig 3  PP.H4 ECDF 多面板：(A) ECDF (B) 阈值命中条形 (C) MR 状态→strong 转换
#   Fig 4  Fuji-plot 式基因组分布（点径∝多效性=关联结局数；known/new 形状）
#   Fig 6  收敛面板：HEIDI/Steiger/GTEx 加 Wilson CI + FinnGen 23 新候选复现散点
#   Fig 7  资源一览（工作流示意 + 摘要矩阵 + 复现覆盖诚实披露 12/21）
# 诚实口径：数字全部与 INTEGRITY_AUDIT/FINAL_STORY 一致。
# =============================================================================
import csv, json, os
...
def fig1():
    ...
    # 输入资源
    box(0.4, 8.7, 2.8, 1.1, "eQTLGen 全血 cis-eQTL\nn=31,684", "#4a7a9b", "#2e4d66")
    box(3.7, 8.7, 2.8, 1.1, "GWAS：T2D / CAD / FBG\nn=655,666 / 296,525 / 58,074", "#4a7a9b", "#2e4d66")
    box(7.0, 8.7, 2.6, 1.1, "LD 参考 1000G EUR\n503 样本", "#4a7a9b", "#2e4d66")
    # 第 1 级：全量对
    box(3.3, 6.9, 3.4, 1.15, "31,373 gene×结局对\n全转录组 cis-MR × coloc 全量扫描", "#1a3a6b", "#0f2445")
    arrow(5.0, 8.7, 5.0, 8.05)
    # 第 2 级：MR 显著
    box(3.3, 5.35, 3.4, 1.1, "4,248 MR 显著对\n(mr_p<0.05, Wald ratio)", "#2e5598", "#1a3a6b")
    arrow(5.0, 6.9, 5.0, 6.45)
    # 第 3 级：coloc
    box(3.3, 3.85, 3.4, 1.1, "coloc.abf (p12=1e-5)\n131 strong 共定位 (PP.H4≥0.8)", C_RED, "#7a0000")
    arrow(5.0, 5.35, 5.0, 4.95)
    # 分支：106 known + 23 new
    box(0.5, 2.2, 3.4, 1.2, "106 个已知 T2D/CAD 位点\n全部重现 (100%)", C_GREEN, "#3f5c35")
    box(6.1, 2.2, 3.4, 1.2, "23 个新候选效应基因\n15 已知位点新效应 + 8 弱候选", C_NEW, "#9a5a1c")
    arrow(4.2, 4.25, 2.2, 3.4, C_GREEN)
    arrow(5.8, 4.25, 7.8, 3.4, C_NEW)
    # 复现
    box(6.1, 0.5, 3.4, 1.2, "独立复现\nGTEx v8：11/12 方向一致\nFinnGen R11：12/12 基因级、5 p<0.05\n(对齐覆盖 12/21=57%)", "#5a5a5a", "#3a3a3a")
    arrow(7.8, 2.2, 7.8, 1.7)

    ax.set_title("全转录组 cis-MR × coloc 图谱：工作流与操作特性标定",
                 fontsize=10, pad=12)
    fig.tight_layout(); fig.savefig(f"{OUT}/20260816_F1_workflow_schematic.png", bbox_inches="tight")
    plt.close(fig)
    print("Fig 1 OK")

# ============================ Fig 2：精度漏斗 + Wilson CI ============================
def fig2():
    rows = list(csv.DictReader(open(f"{RES}/m27_precision_funnel_20260816.csv")))
    # MR p 阈值主曲线（前 8 行为纯 MR p 阈值分层）
    m = rows[:8]
    xs = [f(r["n"]) for r in m]
    ...
    ax.set_xlabel("cis-MR 显著阈值（mr_p < X）", fontsize=9)
    ax.set_ylabel("strong coloc 率（PP.H4≥0.8, %）", fontsize=9)
    ax.set_title("单工具 cis-MR 的操作特性曲线：收紧阈值 → strong coloc 支持率单调校准",
                 fontsize=10)
    # 参考点（非 MR p 阈值分层，虚线注释）
    ref = {r["stratum"]: r for r in rows if r["stratum"] in ("all_QC", "stage2_grid(M5)", "null(>=0.5)")}
    ax.axhline(0.418, color="#bbbbbb", ls=":", lw=1)
    ax.text(7.6, 1.0, "全量 31,371 对：0.42%", fontsize=7, color="#777777", ha="right")
    ax.axhline(12.96, color=C_RED, ls="--", lw=1.2)
    ax.text(7.6, 13.6, "stage-2 clump+IVW 复核网格：12.96%\n[10.83, 15.43]", fontsize=7, color=C_RED, ha="right")
    ax.text(0.02, 0.97, "MR sig 集内：3.04% [2.56, 3.60]\nMR sig 集外（14,294 对）：0.014%（仅 2）",
            transform=ax.transAxes, fontsize=7, va="top", color="#333333",
            bbox=dict(boxstyle="round,pad=0.3", fc="#f0f4fa", ec="#cccccc", lw=0.5))
    ax.set_ylim(0, 30)
    fig.tight_layout(); fig.savefig(f"{OUT}/20260816_F2_precision_funnel_CI.png", bbox_inches="tight")
    plt.close(fig)
    print("Fig 2 OK")
...
def fig3():
    ...
    # (A) ECDF
    ax = axes[0]
    for key, lab, col, ls in [("sig", f"MR 显著 (mr_p<0.05)  n={len(layers['sig']):,}", C_MAIN, "-"),
    ...
    ax.set_xlabel("PP.H4 (coloc.abf)"); ax.set_ylabel("ECDF（累计比例）")
    ax.set_xlim(0, 1); ax.set_ylim(0, 1)
    ax.legend(fontsize=6.5, loc="lower right", frameon=False)
    ax.set_title("(A) PP.H4 全分布按 MR 状态分层", fontsize=9)
    # (B) 阈值命中条形
    ax = axes[1]
    ...
    ax.set_xticks(range(3)); ax.set_xticklabels(["MR 显著", "grey", "null"], fontsize=8)
    ax.set_ylabel("PP.H4 命中率（%）"); ax.set_ylim(0, 8)
    ax.set_title("(B) 阈值命中率（Wilson 95% CI）", fontsize=9)
    ...
    ax.set_ylabel("对 数"); ax.set_ylim(0, max(total)*1.25)
    ax.set_title("(C) MR 状态 → strong 转换", fontsize=9)
    ax.legend(fontsize=6.5, frameon=False, loc="upper right")
    fig.suptitle("全图谱 PP.H4 分布：strong coloc 集中在 MR 显著层", fontsize=10, y=1.0)
    fig.tight_layout(); fig.savefig(f"{OUT}/20260816_F3_pph4_panel.png", bbox_inches="tight")
    plt.close(fig)
    print("Fig 3 OK")
...
def fig4():
    ...
    ax.set_xlabel("染色体 (hg19)", fontsize=9); ax.set_ylabel("GWAS 峰 $-\\log_{10}(p)$", fontsize=9)
    ax.set_title("图谱 129 个 nominal-sig strong coloc 的基因组分布（Fuji-plot 式）\n"
                 "灰区 AP3S2×t2d、ZNF19×cad 未入图（见 R3 vignette）", fontsize=10)
    ax.set_ylim(0, None)
    fig.tight_layout(); fig.savefig(f"{OUT}/20260816_F4_fuji_genome.png", bbox_inches="tight")
    plt.close(fig)
    print("Fig 4 OK, 点数:", len(pts))
...
def fig6():
    ...
    # FinnGen 23 新候选复现（m28，x 轴原始 GWAS p 用 m25 gwas_min_p 补齐）
    ...
    # (a) HEIDI
    ax = axes[0, 0]
    bars = [("stage-2 全集", heidi["heidi_ok_pre"], heidi["n_pre"]),
            ("strong 子集", heidi["heidi_ok_strong"], heidi["n_strong"])]
    for i, (lab, k, n) in enumerate(bars):
        ...
        ax.text(i, 100*k/n + 3, f"{100*k/n:.1f}%\n{n} 对", ha="center", fontsize=7.5)
    ax.set_xticks([0, 1]); ax.set_xticklabels([b[0] for b in bars], fontsize=8)
    ax.set_ylabel("HEIDI 通过率（%）"); ax.set_ylim(0, 95)
    ax.set_title("(A) SMR+HEIDI：strong 子集通过率更高", fontsize=9)
    # (b) Steiger
    ax = axes[0, 1]
    ...
    ax.text(0.0, 6, f"{steig['rev']} 反向（0 反向显著）", ha="center", fontsize=7, color="#555555")
    ax.set_xticks([0]); ax.set_xticklabels(["eQTL→结局"], fontsize=8)
    ax.set_ylim(0, 115); ax.set_ylabel("方向一致占比（%）")
    ax.set_title("(B) Steiger：因果方向验证", fontsize=9)
    # (c) GTEx 独立 eQTL 方向复现（FACTS §5 正式口径：m26 新层 11/12 + M15 已知层 44/63；
    #     tissue_triangulation 55/79 属内部一致检验，仅作补充说明，移出主图）
    ax = axes[1, 0]
    ...
    for i, (lab, k, n) in enumerate([("新 23 候选层", m26_ok, len(m26_meas)),
                                     ("已知 106 层", p1_ok, len(p1_meas))]):
        lo, hi = wilson(k, n)
    ...
    ax.set_xticks([0, 1]); ax.set_xticklabels(["新 23 候选层", "已知 106 层"], fontsize=8)
    ax.set_ylim(0, 100); ax.set_ylabel("方向一致占比（%）")
    ax.set_title("(C) GTEx v8 独立 eQTL 方向复现（Wilson 95% CI）", fontsize=9)
    # (d) FinnGen 23 新候选：orig p vs FinnGen p 复现散点
    ax = axes[1, 1]
    ox, oy, cx, cy = [], [], [], []
    for r in aligned:
        fp = f(r["finn_p"])
        op = gwas_p.get(r["gene"], f(r["orig_p"]))
    ...
    ax.set_xlabel("原始 GWAS 峰 $-\\log_{10}(p)$"); ax.set_ylabel("FinnGen R11 $-\\log_{10}(p)$")
    ax.set_title("(D) FinnGen 独立队列复现（23 新候选）", fontsize=9)
    ax.text(0.98, 0.03, f"可对齐 {len(aligned)}/21 (57%)\n基因级 12/12 方向一致\n★ = FinnGen p<0.05 ({len(sig5)})",
            transform=ax.transAxes, fontsize=7, ha="right", va="bottom",
            bbox=dict(boxstyle="round,pad=0.3", fc="#fdf6ec", ec="#d9822b", lw=0.6))
    fig.suptitle("收敛与独立复现验证（Wilson 95% CI）", fontsize=10, y=1.0)
    fig.tight_layout(); fig.savefig(f"{OUT}/20260816_F6_convergence_ajhg.png", bbox_inches="tight")
    plt.close(fig)
    print("Fig 6 OK")

# ============================ Fig 7：资源一览 ============================
def fig7():
    n_pairs = {"T2D": 10190, "CAD": 14675, "FBG": 6508}
    fig = plt.figure(figsize=(9.5, 5.6), dpi=300)
    gs = fig.add_gridspec(2, 2, width_ratios=[1.25, 1], height_ratios=[1, 0.8])
    ax = fig.add_subplot(gs[0, 0])
    ax.axis("off")
    ax.set_title("(A) 数据来源与版本", fontsize=9, loc="left")
    items = [("eQTLGen (n=31,684, 全血, hg19)", "cis-eQTL + AF, 2018-09-05"),
             ("GWAS T2D (Xue 2018, n=655,666)", "GCST006867"),
             ("GWAS CAD (n=296,525)", "GCST005194"),
             ("GWAS FBG (n=58,074)", "GCST005186"),
             ("1000 Genomes EUR (n=503)", "LD 参考, hg19"),
             ("GTEx v8 (n=838)", "跨组织 eQTL 复现"),
             ("FinnGen R11", "独立结局复现")]
    y = 0.95
    ...
    ax.axis("off")
    ax.set_title("(B) 图谱资源底表（投稿随 Zenodo DOI 发布）", fontsize=9, loc="left")
    data = [
        ["全量图谱", "31,373 对", "gene×结局×MR+coloc 全字段"],
        ["strong 子集", "131", "PP.H4≥0.8（t2d 67 / cad 60 / fbg 4）"],
        ["已知位点重现", "106/106", "100%"],
        ["新候选效应基因", "23", "15 已知位点新效应 + 8 弱候选"],
        ["GTEx 复现", "11/12 方向一致", "1 冲突 (VSIG8)"],
        ["FinnGen 复现", "12/12 基因级", "5 p<0.05；覆盖 12/21=57%"],
    ]
    tab = ax.table(cellText=data, colLabels=["层级", "数量", "说明"], loc="upper center",
    ...
    tab2 = ax.table(cellText=col_data, colLabels=["列", "类型", "示例值"], loc="upper center",
    ...
    ax.text(0.58, 0.62, "诚实披露（复现覆盖）", fontsize=8.5, fontweight="bold", color="#c00000")
    ax.text(0.58, 0.50, "FinnGen R11 对齐覆盖 12/21 (57%)：\n7 个 lead SNP 未在 R11 定位\n(PLAUR/SLC12A3/CWF19L1/U6atac/\nN4BP2L2/ZBTB46/ZNF100)、2 orig 缺数据\n(RPL13/CLEC3B)、2 fbg 无表型跳检", fontsize=6.8, va="top", color="#333333")
    ax.text(0.58, 0.06, "用途：任何 cis-MR 靶点筛选可直接查表。schema 随打包发布\n(DATA_AVAILABILITY §4.1)。候选=假设生成，非因果发现。",
            fontsize=6.8, color="#555555", va="bottom")
    fig.suptitle("图谱资源快照：数据来源、底表结构与独立复现覆盖", fontsize=10, y=1.0)
    fig.tight_layout(); fig.savefig(f"{OUT}/20260816_F7_resource_ajhg.png", bbox_inches="tight")
    plt.close(fig)
    print("Fig 7 OK")
